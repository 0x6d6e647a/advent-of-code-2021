...
foreach my $line0 ( @lines ) {
    foreach my $line1 ( @lines ) {
        next if $line0 eq $line1;

        my $snf0 = sfn_reduce(sfn_parse([split //, $line0]));
        my $snf1 = sfn_reduce(sfn_parse([split //, $line1]));
        my $mag = sfn_magnitude(sfn_reduce(sfn_add($snf0, $snf1,)));
